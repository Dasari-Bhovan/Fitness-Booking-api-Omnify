...
from ..models.booking import Booking, FitnessClass
...
class BookingService:
    ...
    def get_all_classes(self, timezone: Optional[str] = "Asia/Kolkata") -> List[FitnessClass]:
        ...
        try:
            ...
            classes = (
                self.db.query(FitnessClass)
                .filter(
                    and_(FitnessClass.is_active == True, FitnessClass.class_datetime > current_time)
                )
                .order_by(FitnessClass.class_datetime)
                .all()
            )
    ...
    def get_class_by_id(self, class_id: int) -> FitnessClass:
        ...
        fitness_class = (
            self.db.query(FitnessClass)
            .filter(FitnessClass.id == class_id, FitnessClass.is_active == True)
            .first()
        )

        if not fitness_class:
            raise ClassNotFoundException(
                f"Class with ID {class_id} not found or inactive", {"class_id": class_id}
            )
        fitness_class.class_datetime = tz_manager.convert_timezone(
    ...
    def get_bookings_by_email(self, client_email: str) -> List[Booking]:
        ...
        try:
            bookings = (
                self.db.query(Booking)
                .filter(Booking.client_email == client_email)
            ...
            sample_classes = [
                {
                    ...
                    "class_datetime": tz_manager.get_one_day_aheadtime().replace(
